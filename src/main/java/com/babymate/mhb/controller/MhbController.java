...
public class MhbController {
    ...
    @PostMapping("insert")
    public String insert(@Valid @ModelAttribute("mhbVO") MhbVO mhbVO,
    ...
    @PostMapping("update")
    public String update(@Valid @ModelAttribute("mhbVO") MhbVO mhbVO,
                         BindingResult result,
                         ModelMap model,
        ...
        if (result.hasErrors()) return "admin/mhb/update_mhb_input";
    ...
    public void photoDeleted(@PathVariable Integer id, jakarta.servlet.http.HttpServletResponse resp) throws IOException {
        byte[] bytes = mhbSvc.getPhotoBytesRaw(id); // 你在 service 用 nativeQuery 直接撈
        if (bytes == null) { resp.setStatus(404); return; }
    ...
    private static String detectImageMimeType(byte[] data) {
        if (data.length >= 8 && (data[0] & 0xFF) == 0x89 && data[1] == 0x50 && data[2] == 0x4E &&
            data[3] == 0x47 && data[4] == 0x0D && data[5] == 0x0A && data[6] == 0x1A && data[7] == 0x0A) return "image/png";
        if (data.length >= 3 && (data[0] & 0xFF) == 0xFF && (data[1] & 0xFF) == 0xD8) return "image/jpeg";
    ...
    public BindingResult removeFieldError(MhbVO mhbVO, BindingResult result, String removedFieldname) {
        List<FieldError> keep = result.getFieldErrors().stream()
                .filter(e -> !e.getField().equals(removedFieldname))
